...
def categoryItems():
    ...
    return render_template('index.html', category=None, items=items, allcategory = allcategory,
            user_email = login_session.get('email'), username = login_session.get('username'), usr_img = login_session.get('picture'))

@app.route('/sales/<string:name>')
def selectCategory(name):
    allcategory = session.query(Sales_Category)
    category = session.query(Sales_Category).filter_by(name = name).one()
    items = session.query(Sales_Item).filter_by(category_id = category.category_id)
    return render_template('index.html', category=category, items=items, allcategory = allcategory,
             user_email = login_session.get('email'),username = login_session.get('username'), usr_img = login_session.get('picture'))

@app.route('/sales/<string:name>/JSON')
def selectCategoryJSON(name):
    category = session.query(Sales_Category).filter_by(name = name).one()
    items = session.query(Sales_Item).filter_by(category_id = category.category_id).all()
...
def newItem(name):
    ...
    if request.method == 'POST':
       salesItem = Sales_Item(name = request.form['name'], description=request.form['description'], category_id = category.category_id,
                                        price=request.form['price'], created_by = login_session['email'],created_on = datetime.now(), user_email= login_session['email'])
...
@app.route('/sales/<string:name>/<int:item_id>/delete/', methods=['GET','POST'])
...
def Createuser(login_session):
    newuser = User_Details(user_email = login_session.get('email'), first_name = login_session.get('username'), img = login_session.get('picture'))
    session.add(newuser)
    session.commit()
